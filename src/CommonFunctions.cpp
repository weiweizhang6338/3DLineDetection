...
void LineFunctions::lineFitting(int rows, int cols,
  ...
  for (int i = 0; i < straightString.size(); ++i) {
    ...
    lineFittingSVD(&straightString[i][0], straightString[i].size(), parameters,
                   maxDev);
    // if ( isOK )
    {
...
void LineFunctions::subDivision(
  ...
  // Compute the length of the straight line segment defined by the endpoints of
  // the cluster.
  ...
  for (int i = first_index, count = contour.size(); i != last_index;
    ...
    double deviation =
        static_cast<double>(abs(((current.x - first.x) * (first.y - last.y)) +
  ...
  // deviation. 	float ratio = length / std::max( max_deviation, min_deviation
  // );
...
void LineFunctions::lineFittingSVD(cv::Point *points, int length,
...
void PCAFunctions::Ori_PCA(PointCloud<double> &cloud, int k,
                           std::vector<PCAInfo> &pcaInfos, double &scale,
                           double &magnitd) {
...
void PCAFunctions::MCMD_OutlierRemoval(
  ...
  for (int j = 0; j < pcaInfo.idxIn.size(); ++j) {
    ...
    h_mean +=
        cv::Matx31d(pointData[idx][0], pointData[idx][1], pointData[idx][2]);
  }
