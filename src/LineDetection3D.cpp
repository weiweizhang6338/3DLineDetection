  ...
  for (int i = 0; i < idx; ++i) {
    ...
    while (count < clusterTemp.size()) {
      ...
      for (int j = 0; j < num; ++j) {
        ...
        double dOrtho = abs(dx * normalCur.val[0] + dy * normalCur.val[1] +
                            dz * normalCur.val[2]);
        if (dOrtho > thOrtho) {
          continue;
        }
  ...
  for (int i = 0; i < patches.size(); ++i) {
    ...
    for (int j = 0; j < pointAdjacentTemp.size(); ++j) {
      ...
      vector<int>::iterator new_end =
          unique(pointAdjacentTemp[j].begin(), pointAdjacentTemp[j].end());
...
#pragma omp parallel for
  for (int i = 0; i < patches.size(); ++i) {
    ...
    for (int j = 0; j < patches[i].idxAll.size(); ++j) {
      ...
      cv::Mat_<double> pt3d = (cv::Mat_<double>(3, 1) << pointData.pts[id].x,
    ...
    std::sort(ptScales.begin(), ptScales.end(),
...
bool LineDetection3D::maskFromPoint(std::vector<cv::Point2d> &pts2d,
                                    double radius, double &xmin, double &ymin,
                                    double &xmax, double &ymax, int &margin,
                                    cv::Mat &mask) {
...
void LineDetection3D::lineFromMask(
    cv::Mat &mask, int thLineLengthPixel,
  ...
  for (int i = 0; i < contours.size(); ++i) {
    ...
    LineFunctions::lineFitting(mask2.rows, mask2.cols, contours[i], thLength,
                               lineTemp);
  ...
  for (int i = 0; i < planes.size(); ++i) {
    ...
    for (int m = 0; m < planes[i].lines3d.size(); ++m) {
      ...
      for (int n = 0; n < planes[i].lines3d[m].size(); ++n) {
        cv::Mat orientTemp =
            cv::Mat(planes[i].lines3d[m][n][1] - planes[i].lines3d[m][n][0]);
...
void LineDetection3D::lineMerging(
  ...
  for (int i = 0; i < lineInfos.size(); ++i) {
    ...
    for (int j = 0; j < idHyps.size(); ++j) {
      ...
      double disOrtho1 = sqrt(vOrtho1.x * vOrtho1.x + vOrtho1.y * vOrtho1.y +
                              vOrtho1.z * vOrtho1.z);
      ...
      double disOrtho2 = sqrt(vOrtho2.x * vOrtho2.x + vOrtho2.y * vOrtho2.y +
                              vOrtho2.z * vOrtho2.z);
